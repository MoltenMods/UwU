...
namespace UwU.Controls
{
    public class Button
    {
        ...
        private Button(string text, Vector2 size, Vector3 position)
        {
            ...
            Position.DistanceFromEdge = position;
            Position.updateAlways = true;
            _spriteRenderer.size = size;
            _boxCollider2D.size = size;
        }

        public static async Task<Button> Create(string text, Vector2 size, Vector3 position)
        {
        ...
        public static async Task<Button> Create(string text, Vector2 size, Vector2 position)
        {
            ...
            return new Button(text, size, new Vector3(position.x, position.y, -100f));
        }
